...
def run(modflow_exe, data_path, modflow_nam, modflow_wel, model_prj, xll, yll, rotation, geo_prj, contour_levels,
        shape_type, details, stream_transformation_name='', std_minimum_change=0, export_layer_string="",
        export_sp_string="", package=""):
    """
    Run MODFLOW model
    ...
    if has_package:
        ...
        # streamflow_out_old_list = extract_modflow_data(budget_old, package, ml.dis.nper, ml.dis.nlay)
    ...
    for feature in data['features']:
        ...
        flow_factor = float(feature['properties'].get('Pumpfactor', 1))
                                    ...
                                    if streamflow_out_diff[layer][sp][row][col] != 0:
                                        ...
                                        if percentage_changes >= float(std_minimum_change):
                                            ...
                                            if details == 'All':
                                                ...
                                                atr_flowout = {'id': node_id,
                                                               'SF_Diff':
                                                                   float_custom(streamflow_depletion * 7.4805/24/60),
                                                               'layer': layer + 1,
                                                               'stress_period': sp + 1,
                                                               'ij': str(row + 1) + "," + str(col + 1)}
