...
class StreamDepletionWorkflow(object):
    """
    Class Based Controller for pump impact tool
    """
    WORKFLOW_ID = 'stream_depletion'
    ...
    def __init__(self, user, workspace, session_id, xll, yll, rotation, db_dir, model_units, model_version, modflow_exe,
                 ...
                 std_minimum_change, export_layer_string, export_sp_string):
        """
        Constructor.
        ...
        self.job_name = 'stream_depletion'
    ...
    def prepare(self):
        ...
        prepare_stream_depletion_job = CondorWorkflowJobNode(
            name='stream_depletion_job',
            workflow=self.workflow,
            condorpy_template_name='vanilla_transfer_files',
            remote_input_files=[
                os.path.join(job_executables_dir, 'stream_depletion_executable.py'),
                self.db_dir
            ],
            attributes=dict(
                executable='stream_depletion_executable.py',
            )
        )

        if not self.std_minimum_change:
            # Setting appropriate arguments for the prepare_stream_depletion executable
            session = None
            try:
        ...
        prepare_stream_depletion_job.set_attribute('arguments', (
            self.modflow_exe,
        ...
        prepare_stream_depletion_job.set_attribute('transfer_input_files', ('../{0}'.format(input_archive_filename),))
        prepare_stream_depletion_job.save()
